var diabcu = (function() {
	...
	var dayDetails = function(readings, target) {
		...
		for(var i = readings.length - 1; i >= 0; i--) {
			...
			// possibly based the padding-left and the width on the time of the day and number of readings on the time of the day.
			list += format('<li><div class="bg" style="background: {0}>{1}</div><div class="time">{2}</div></li>',color,bg,new Date(readings[i].timestamp).toTimeString().substring(0,5));
		}

		return format("<div class='date'>{0}</div><ul>{1}</ul>",new Date(readings[0].timestamp).toUTCString().substring(0,16), list);
	};
	...
	var days = function(days, target) {
		...
		while(true) {
			...
			dots.push("<div class='day-details'></div>");
			if(bail) break;
		}
		return dots;
	};
